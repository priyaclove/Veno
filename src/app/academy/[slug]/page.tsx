...
interface IntelligenceData {
  academyServices: academyService[];
}

// Helper function to get data from JSON file
function getIntelligenceData(): IntelligenceData {
  const filePath = path.join(process.cwd(), 'src/data/AcademyData.json');
  const fileData = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(fileData);
}

// Helper function to get a specific service by slug
function getServiceBySlug(slug: string): academyService | undefined {
  const data = getIntelligenceData();
  return data.academyServices.find(service => service.slug === slug);
}
...
export async function generateStaticParams() {
  ...
  if (!data?.academyServices || !Array.isArray(data.academyServices)) {
    throw new Error('Missing or invalid academyServices data');
  }

  return data.academyServices.map(service => ({
    slug: service.slug,
  }));
}
...
// Main page component
export default function academyServicesPage({ params }: { params: { slug: string } }) {
  const service = getServiceBySlug(params.slug);
