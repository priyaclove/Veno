...
function getIntelligenceData(): IntelligenceData {
  const filePath = path.join(process.cwd(), 'src/data/hr-services.json');
  const fileData = fs.readFileSync(filePath, 'utf8');
  return JSON.parse(fileData);
}

// Helper function to get a specific service by slug
function getServiceBySlug(slug: string): hrService | undefined {
  const data = getIntelligenceData();
  return data.hrServices.find(service => service.slug === slug);
}
...
export async function generateStaticParams() {
  ...
  if (!data?.hrServices || !Array.isArray(data.hrServices)) {
    throw new Error('Missing or invalid hrServices data');
  }

  return data.hrServices.map(service => ({
    slug: service.slug,
  }));
}
...
// Main page component
export default function hrServicesPage({ params }: { params: { slug: string } }) {
  const service = getServiceBySlug(params.slug);
